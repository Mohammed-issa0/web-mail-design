...
export default function MailApp() {
  ...
  const [rightSidebarView, setRightSidebarView] = useState<RightSidebarView>(null)
  const [showNewMessage, setShowNewMessage] = useState(false)
  const [showSettings, setShowSettings] = useState(false)
  const [viewMode, setViewMode] = useState<"list" | "split">("list")
  const [mobileView, setMobileView] = useState<"inbox" | "tasks" | "compose" | "calendar" | "contacts">("inbox")
  ...
  const handleMobileViewChange = (view: "inbox" | "tasks" | "compose" | "calendar" | "contacts") => {
    setMobileView(view)
    if (view === "compose") {
      ...
      <div className="flex-1 flex overflow-hidden pb-16 md:pb-0">
        ...
        {showSettings ? (
          ...
          <>
            ...
            {selectedEmail && (
              <MailDetail email={selectedEmail} onBack={handleBack} onReply={() => setShowNewMessage(true)} />
            )}
          </>
        )}
        ...
        {!showSettings && (
          <>
            <div className="hidden md:block">
              <RightSidebar activeView={rightSidebarView} onViewChange={setRightSidebarView} />
            </div>

            {rightSidebarView === "calendar" && <CalendarPanel onClose={() => setRightSidebarView(null)} />}

            {rightSidebarView === "tasks" && <TasksPanel onClose={() => setRightSidebarView(null)} />}

            {rightSidebarView === "contacts" && <ContactsPanel onClose={() => setRightSidebarView(null)} />}
          </>
        )}
      </div>
